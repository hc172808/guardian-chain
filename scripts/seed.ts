...
async function seed() {
  ...
  await db.insert(schema.featureTogglesTable).values([
    { feature_key: "defi_swap", feature_name: "DeFi Swap", description: "Enable token swapping in the DeFi section.", is_enabled: true, admin_only: false },
    { feature_key: "faucet", feature_name: "Faucet", description: "Enable the GYDS test faucet.", is_enabled: true, admin_only: false },
    { feature_key: "token_launch", feature_name: "Token Launch", description: "Allow new token launches via bonding curve.", is_enabled: true, admin_only: false },
    { feature_key: "smart_contracts", feature_name: "Smart Contracts", description: "Enable smart contract editor and deployer.", is_enabled: true, admin_only: false },
    { feature_key: "node_terminal", feature_name: "Node Terminal", description: "Admin-only node terminal access.", is_enabled: true, admin_only: true },
    { feature_key: "analytics_v2", feature_name: "Analytics V2", description: "Next-gen analytics dashboard (beta).", is_enabled: false, admin_only: false },
  ]);
  ...
  await db.insert(schema.transactionsTable).values([
    ...
    {
      hash: "0x789ghi012jkl789ghi012jkl789ghi012jkl789ghi012jkl789ghi012jkl789g",
  ...
  const ended = new Date(now.getTime() - 5 * 24 * 60 * 60 * 1000);
  await db.insert(schema.tokenLaunchesTable).values([
    {
      ...
      raised_amount: 312_400,
      initial_price: 0.001,
      max_price: 0.01,
      bonding_curve_type: "linear",
      bonding_curve_steepness: 1.2,
      participants: 847,
      is_premier: true,
      starts_at: ended,
      ends_at: soon,
    },
    {
      ...
      creator_id: "system",
      status: "active",
      target_raise: 200_000,
      raised_amount: 78_600,
      initial_price: 0.0005,
      max_price: 0.005,
      bonding_curve_type: "exponential",
      bonding_curve_steepness: 1.5,
      participants: 231,
      is_premier: false,
      starts_at: ended,
      ends_at: soon,
