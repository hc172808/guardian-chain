...
import (
        ...
        headersync "github.com/gydschain/litenode/header-sync"
)
...
func main() {
        // Check for "setup" sub-command to launch the web wizard
        if len(os.Args) >= 2 && os.Args[1] == "setup" {
                wizardPort := 8888
                // Parse --port flag if provided
                for i := 2; i < len(os.Args)-1; i++ {
                        if os.Args[i] == "--port" {
                                fmt.Sscanf(os.Args[i+1], "%d", &wizardPort)
                        }
                }
                fmt.Printf(`
╔═══════════════════════════════════════════════════════════════╗
║           GYDS Lite Node v%s — Setup Wizard               ║
║      Launching web configuration interface...               ║
╚═══════════════════════════════════════════════════════════════╝
`, version)
                startSetupWizard(wizardPort, version)
                return
        }
...
func (n *LiteNode) handleHealth(w http.ResponseWriter, r *http.Request) {
        w.Header().Set("Content-Type", "application/json")
        fmt.Fprintf(w, `{"status":"ok","node":"gyds-litenode","version":"%s"}`, version)
}
