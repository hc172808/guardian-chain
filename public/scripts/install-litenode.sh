...
#  Clones from: https://github.com/hc172808/litenode.git
...
REPO_URL="${REPO_URL:-https://github.com/hc172808/litenode.git}"
REPO_DIR="${REPO_DIR:-$HOME/gyds-litenode}"
...
# ─── 0. Clone / update repo ──────────────────────────────────────────────────
log "[0/6] Fetching litenode source from ${REPO_URL}..."
...
if [[ -f "$REPO_DIR/go.mod" ]]; then
    SRC_DIR="$REPO_DIR"
elif [[ -d "$REPO_DIR/cmd/litenode" ]]; then
    SRC_DIR="$REPO_DIR"
else
    err "Cannot find go.mod in ${REPO_DIR}. Check repo layout."; exit 1
fi
...
if ! command -v go >/dev/null || [[ "$(go version 2>/dev/null | awk '{print $3}')" < "go1.22" ]]; then
    if [[ "$(uname)" == "Linux" ]]; then
        cd /tmp
        ...
        err "Please install Go 1.22+ manually (brew install go on macOS)."; exit 1
    fi
fi
log "    $(go version)"
...
BUILD_TMP="$(mktemp -d)"
cp -r "$SRC_DIR" "$BUILD_TMP/litenode-src"

if [[ -d "$BUILD_TMP/litenode-src/cmd/litenode" ]]; then
    CMD_PATH="./cmd/litenode"
else
    CMD_PATH="."
fi

( cd "$BUILD_TMP/litenode-src" && go mod download && go build -ldflags "-s -w -X main.Version=${GYDS_VERSION}" -o "$GYDS_BIN/gyds-litenode" $CMD_PATH )
rm -rf "$BUILD_TMP"
