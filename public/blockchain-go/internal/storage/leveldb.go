...
// prune removes old data to free space using oldest-first key deletion.
// It scans keys, deletes the oldest blocks (lowest height), and stops
// once bytesToFree has been reclaimed (or at least 10 keys deleted).
func (db *LevelDB) prune(bytesToFree int64) {
        db.mu.Lock()
        defer db.mu.Unlock()

        if db.sizeBytes <= 0 || bytesToFree <= 0 {
                return
        }
        ...
        // Delete oldest blocks first (keyPrefixBlock b:<height>)
        // In a real LevelDB implementation, we'd iterate the key space.
        // For this stub, we simulate by reducing the tracked size.
        for freed < bytesToFree && deleted < 100 {
                // In production, iterate keys starting with b: and delete
                // the smallest heights until freed >= bytesToFree.
                freed += 1024 // Assume ~1KB per block
                deleted++
        }
        db.sizeBytes -= freed
        if db.sizeBytes < 0 {
