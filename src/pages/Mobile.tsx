...
const MobileHome = () => {
  ...
  const quickActions = [
    { icon: Send, label: 'Send', color: 'text-blue-400', bg: 'bg-blue-400/10', path: '/wallet' },
    { icon: Download, label: 'Receive', color: 'text-green-400', bg: 'bg-green-400/10', path: '/wallet' },
    { icon: ArrowLeftRight, label: 'Swap', color: 'text-purple-400', bg: 'bg-purple-400/10', path: '/defi' },
    { icon: Globe, label: 'Bridge', color: 'text-amber-400', bg: 'bg-amber-400/10', path: '/defi' },
    { icon: TrendingUp, label: 'Stake', color: 'text-cyan-400', bg: 'bg-cyan-400/10', path: '/defi' },
    { icon: Droplets, label: 'Faucet', color: 'text-pink-400', bg: 'bg-pink-400/10', path: '/faucet' },
  ];
      ...
      <div className="relative overflow-hidden rounded-2xl bg-gradient-to-br from-primary/80 via-primary/60 to-primary/40 p-5">
        ...
        <div className="flex gap-3 mt-4">
          <div className="flex-1 bg-white/10 rounded-lg p-2 text-center">
            <p className="text-xs text-white/60">GYDS</p>
            <p className="font-bold text-white text-sm">0.00</p>
          </div>
          <div className="flex-1 bg-white/10 rounded-lg p-2 text-center">
            <p className="text-xs text-white/60">GYD</p>
            <p className="font-bold text-white text-sm">0.00</p>
          </div>
          <div className="flex-1 bg-white/10 rounded-lg p-2 text-center">
...
const MobileDefi = () => {
  ...
  return (
    <div className="space-y-4">
      {[
        { label: 'Swap Tokens', icon: ArrowLeftRight, desc: 'Instantly swap between GYDS and GYD', color: 'text-purple-400 bg-purple-400/10' },
        { label: 'Liquidity Pools', icon: Droplets, desc: 'Add liquidity and earn trading fees', color: 'text-blue-400 bg-blue-400/10' },
        { label: 'Stake GYDS', icon: TrendingUp, desc: 'Earn rewards by staking your GYDS', color: 'text-green-400 bg-green-400/10' },
        { label: 'Order Book', icon: Activity, desc: 'Limit and market orders', color: 'text-amber-400 bg-amber-400/10' },
        { label: 'Yield Vaults', icon: Zap, desc: 'Auto-compound strategies', color: 'text-cyan-400 bg-cyan-400/10' },
        { label: 'Cross-Chain Bridge', icon: Globe, desc: 'Bridge from 25 external chains', color: 'text-pink-400 bg-pink-400/10' },
      ].map(item => (
        <button
          key={item.label}
          ...
          className="w-full flex items-center gap-4 p-4 rounded-xl bg-card border border-border hover:border-primary/40 transition-all text-left active:scale-[0.98]"
        >
          <div className={cn('p-2.5 rounded-lg', item.color.split(' ')[1])}>
    ...
    <div className="space-y-4">
      ...
      {[
        ...
        <button key={item.label} onClick={() => navigate(item.path)}
          className="w-full flex items-center gap-4 p-4 rounded-xl bg-card border border-border text-left active:scale-[0.98]"
        >
          <div className="p-2.5 rounded-lg bg-primary/10">
            <item.icon className="h-5 w-5 text-primary" />
          </div>
  ...
  return (
    <div className="space-y-4">
      {[
        ...
        { label: 'Protocol Docs', icon: Settings, path: '/protocol' },
      ].map(item => (
        <button key={item.label} onClick={() => navigate(item.path)}
          className="w-full flex items-center gap-4 p-4 rounded-xl bg-card border border-border text-left active:scale-[0.98]"
        >
      ...
      <div className="mt-2 pt-4 border-t border-border space-y-3">
        <button
          onClick={() => navigate('/')}
          className="w-full flex items-center gap-3 p-3 rounded-xl bg-secondary/30 text-sm text-muted-foreground hover:text-foreground transition-colors"
        >
