...
const Auth = () => {
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast({ title: 'Failed to send reset email', description: err?.message || 'Unknown error', variant: 'destructive' });
    }
    setIsLoading(false);
  };
      ...
      if (isLogin) {
        try {
          ...
          if (result?.status === 'complete') {
            ...
            toast({ title: 'Login Incomplete', description: 'Additional verification may be required.', variant: 'destructive' });
          }
        } catch (err: any) {
          toast({
            title: 'Login Failed',
            description: err?.errors?.[0]?.longMessage || err?.errors?.[0]?.message || 'Invalid email or password.',
            variant: 'destructive',
          });
        }
      } else {
        try {
          ...
          if (result?.status === 'complete') {
            ...
            toast({ title: 'Account Created!', description: 'Welcome to ChainCore! You are now logged in.' });
            navigate('/');
          } else {
