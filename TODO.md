...
| BC-7 | ✅ | Block-confirmation status (pending → confirmed via real RPC) | New `src/lib/txConfirmation.ts` — `submitAndAwaitConfirmation()` inserts the row with `status='pending'`, polls `eth_getTransactionReceipt` every 2 s on the active network's RPC, then flips the row to `confirmed` (status 0x1), `failed` (status 0x0), or `failed` after 60 s timeout. Wired into Wallet → Send and DeFi → Swap; both surface a "⏳ Pending" toast immediately and a follow-up "✅ Confirmed" / "❌ Failed" toast when the receipt lands. Existing `useTransactionNotifications` realtime listener already turns the DB row update into a UI notification, so no extra channel needed. |
...
| MIN-2 | ✅ | Audit reward distribution writes | New `src/lib/miningRewards.ts` provides a single, deterministic, integer-only `distributeBlockReward()` that emits one `token_operations` row per recipient credited from `MINING_POOL_WALLET` (`0x…0002`). The pool fee row sweeps any integer-division dust so the sum of credits exactly equals `blockReward` (invariant asserted in code). 13 deterministic tests in `src/lib/miningRewards.test.ts` (vitest) cover solo, pool 50/50, pool pro-rata, fee math, dust handling, address/weight/fee validation, plus a 50-trial property test asserting sum-equality. |
...
| INF-5 | ✅ | Public `/bootnodes.json` endpoint | New `supabase/functions/bootnodes/index.ts` (verify_jwt=false). `GET /functions/v1/bootnodes` returns `{ version, generated_at, networks: { mainnet, testnet, devnet } }`; each network entry has `chain_id` + `bootnodes[]`. Reads from `admin_config.bootnodes` via the service-role key, falls back to a baked-in safe default per network if the row is missing/invalid. Validates each enode URL (`enode://<128-hex>@host:port`). Supports `?network=mainnet` for a single-network response so install scripts can `curl -s …/bootnodes?network=mainnet`. 60 s edge cache. |
...
| SEC-4 | ✅ | Rate-limit Faucet server-side (Supabase RLS or edge function) | `supabase/functions/faucet-claim/index.ts` enforces a hard 24 h cooldown per `(user_id, token_type)` AND per `(wallet_address, token_type)` against the `faucet_claims` table. Validates wallet against `^0x[a-f0-9]{40}$`, refuses to drip when `emergency_shutdown` authority is OFF (`423 chain_halted`), records IP + tx_hash, then inserts the credit into `token_operations`. RLS: users read own rows, founders/admins read all, only service role inserts. Migration `20260505001429`. |
| SEC-5 | ✅ | Linter cleanup + faucet hardening (2026-05-06) | `v_authority_summary` rebuilt as `security_invoker`. `EXECUTE` revoked from PUBLIC/anon/authenticated on `has_role`, `handle_new_user`, `handle_new_user_role` (RLS + triggers still work). Storage buckets `token-logos` / `token-sites` no longer enumerable — owner-only listing policies. Leaked-password protection (HIBP) enabled. `faucet-claim` switched to `verify_jwt=true` and now rejects anonymous JWTs / wrong roles. Linter re-run: 0 issues. |
| SEC-6 | ✅ | Server install requirements doc | `public/docs/SERVER_INSTALL_REQUIREMENTS.md` — Ubuntu 22.04 recipe covering CloudPanel (frontend), Go 1.22, Postgres 15, Docker, UFW + Fail2ban + WireGuard, SSL, systemd, bootnode discovery. Designed to brief a second AI dev or sysadmin. |
